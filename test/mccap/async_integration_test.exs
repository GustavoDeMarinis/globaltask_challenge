defmodule Mccap.AsyncIntegrationTest do
  use MccapWeb.ConnCase, async: false # Oban execution and PG notifications might need async: false
  use Oban.Testing, repo: Mccap.Repo

  alias Mccap.CreditApplications
  alias Mccap.Workers.FetchProviderDataWorker
  alias Mccap.Workers.RiskEvaluationWorker
  ...
  setup %{conn: conn} do
    token = MccapWeb.Token.sign!(%{"role" => "admin"})
