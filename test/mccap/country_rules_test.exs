defmodule Mccap.CountryRulesTest do
  ...
  describe "resolve/1" do
    test "returns {:ok, module} for each supported country" do
      assert {:ok, Mccap.CountryRules.ES} = CountryRules.resolve("ES")
      assert {:ok, Mccap.CountryRules.PT} = CountryRules.resolve("PT")
      assert {:ok, Mccap.CountryRules.IT} = CountryRules.resolve("IT")
      assert {:ok, Mccap.CountryRules.MX} = CountryRules.resolve("MX")
      assert {:ok, Mccap.CountryRules.CO} = CountryRules.resolve("CO")
      assert {:ok, Mccap.CountryRules.BR} = CountryRules.resolve("BR")
    end
