defmodule Mccap.CreditApplications do
  @moduledoc """
  ...
  business rules, delegates to `Mccap.CountryRules` for per-country
  ...
  def create_application(attrs) do
    ...
    |> Multi.insert(:audit_log, fn %{app: app} ->
      Mccap.CreditApplications.AuditLog.changeset(%Mccap.CreditApplications.AuditLog{}, %{
        old_status: nil,
        new_status: "created",
        actor: "system",
    ...
    |> case do
      {:ok, %{app: app}} ->
        app = Repo.preload(app, audit_logs: from(a in Mccap.CreditApplications.AuditLog, order_by: [asc: a.inserted_at]))
        Phoenix.PubSub.broadcast(Mccap.PubSub, "credit_applications", {:new_application, app})
        {:ok, app}
  ...
  def get_application(id) do
    case Ecto.UUID.cast(id) do
      {:ok, uuid} ->
        ...
        case Cachex.get(:mccap_cache, uuid) do
          ...
          _ ->
            case Repo.get(CreditApplication, uuid) do
              ...
              app ->
                app = Repo.preload(app, audit_logs: from(a in Mccap.CreditApplications.AuditLog, order_by: [asc: a.inserted_at]))
                Cachex.put(:mccap_cache, uuid, app, ttl: :timer.minutes(5))
                {:ok, app}
            end
        end
  ...
  def list_applications(filters \\ %{}) do
    ...
    |> Mccap.Pagination.paginate(Repo, filters)
  end
  ...
  def update_provider_payload_and_enqueue_risk(%CreditApplication{} = app, payload) when is_map(payload) do
    ...
    |> Oban.insert(:enqueue_risk_check, Mccap.Workers.RiskEvaluationWorker.new(%{"application_id" => app.id}))
    |> Repo.transaction()
    |> case do
      {:ok, %{update_payload: updated_app}} ->
        app_to_return = invalidate_cache(updated_app)
        Phoenix.PubSub.broadcast(Mccap.PubSub, "credit_applications", {:application_updated, app_to_return})
        Phoenix.PubSub.broadcast(Mccap.PubSub, "credit_application:#{app_to_return.id}", {:application_updated, app_to_return})
        {:ok, app_to_return}
  ...
  def enqueue_fetch_and_increment_attempts(%CreditApplication{} = app) do
    ...
    |> Oban.insert(:enqueue_fetch, Mccap.Workers.FetchProviderDataWorker.new(%{"application_id" => app.id}))
    |> Repo.transaction()
    |> case do
  ...
  def update_status(%CreditApplication{} = app, new_status, actor \\ "system") do
    multi =
      ...
      |> Multi.run(:country_hook, fn _repo, %{status_change: updated_app} ->
        case Mccap.CountryRules.resolve(updated_app.country) do
          {:ok, module} ->
    ...
    multi =
      if new_status in ["approved", "rejected"] do
        Oban.insert(multi, :enqueue_webhook, Mccap.Workers.SendWebhookWorker.new(%{"application_id" => app.id, "status" => new_status}))
      else
        multi
      end
  ...
  defp invalidate_cache(%CreditApplication{id: id} = app) do
    # 1. Borra el dato de la memoria RAM (Cachex)
    Cachex.del(:mccap_cache, id)
     # 2. Vuelve a cargar los datos "frescos" de las asociaciones (audit_logs)
    Repo.preload(app, [audit_logs: from(a in Mccap.CreditApplications.AuditLog, order_by: [asc: a.inserted_at])], force: true)
  end
end
