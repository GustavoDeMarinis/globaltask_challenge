defmodule Mccap.Workers.RecoverStaleApplicationsWorker do
  @moduledoc """
  ...
  def perform(_job) do
    ...
    stale_apps = Mccap.CreditApplications.list_recoverable_applications()
    ...
    if Enum.any?(stale_apps) do
      ...
      Enum.each(stale_apps, fn app ->
        if app.fetch_attempts >= 3 do
          ...
          Mccap.CreditApplications.update_status(app, "provider_timeout")
        else
          Mccap.CreditApplications.enqueue_fetch_and_increment_attempts(app)
        end
      end)
    else
